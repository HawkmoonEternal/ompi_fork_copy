...
int MPI_Session_get_res_change(MPI_Session session, int *type, char *delta_pset, int *incl, int *status, MPI_Info *info_used){
    ...
    if (!flag) {
        rc = ompi_instance_get_res_change(session, NULL, &ompi_rc_op_type, delta_psets, &ndelta_psets, incl, status, (opal_info_t**)info_used, false);
    }else{
        rc = ompi_instance_get_res_change(session, bound_pset, &ompi_rc_op_type, delta_psets, &ndelta_psets, incl, status, (opal_info_t**)info_used, false);
    }
    ...
    *type = MPI_OMPI_CONVT_RC_OP(ompi_rc_op_type);
    ...
    if(NULL != delta_psets){
        strcpy(delta_pset, delta_psets[0]);
