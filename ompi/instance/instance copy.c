...
int send_collective_data_query(pmix_proc_t *procs, size_t nprocs, pmix_query_t *query, size_t nqueries, pmix_info_t *results, size_t nresults);
...
void enter_collective_provider( pmix_status_t status, const pmix_proc_t *source, pmix_info_t info[], size_t ninfo, pmix_info_t results[], size_t nresults, pmix_event_notification_cbfunc_fn_t cbfunc, void *cbdata);
...
void enter_collective_provider( pmix_status_t status, const pmix_proc_t *source, pmix_info_t info[], size_t ninfo, pmix_info_t results[], size_t nresults, pmix_event_notification_cbfunc_fn_t cbfunc, void *cbdata){
    ...
    pmix_info_t *coll_params_info = NULL, *coll_results_info = NULL, *info_ptr, *info = NULL;
    
    ompi_results_type_t results_type = OMPI_RESULTS_NONE;
    ompi_parameters_type_t params_type = OMPI_PARAMS_NONE;
    ompi_function_type_t function_type = OMPI_FUNC_NONE;

    /* TODO: Move to seperate deserializer function*/
    for(n = 0; n < ninfo; n++){
        if(PMIX_CHECK_KEY(&_info[n], "ompi.collective")){
            ret = ompi_collective_deserialize(&coll_in, info);
        }
    }
    ...
    coll = search_pending_collectives(&ompi_instance_pending_collectives, function_type, coll_procs, coll_params);
    ...
    if(NULL == coll){
        ...
        ompi_instance_collective_assign(coll, coll_procs, function_type, coll_params, coll_results, NULL, NULL);

        opal_list_append(&ompi_instance_pending_collectives, &coll->super);
     ...
     */
        ...
        coll->coll_results = coll_results;
        execute_collective_callback(coll);

        coll_params_release(coll_params);
        coll_procs_release(coll_procs);
...
int send_collective_data_query(pmix_proc_t *procs, size_t nprocs, pmix_query_t *query, size_t nqueries, pmix_info_t *results, size_t nresults){
    ...
    coll = OBJ_NEW(ompi_instance_collective_t);
    ompi_instance_collective_assign(coll, coll_procs, OMPI_FUNC_PMIX_QUERY, coll_params, coll_results, NULL, NULL);
...
int recv_collective_data_query(pmix_proc_t *procs, size_t nprocs, pmix_query_t *query, size_t nqueries, pmix_info_cbfunc_t cbfunc, void *cbdata){
    ...
    coll_cbfunc_load_info(coll_cbfunc, cbfunc);

    coll = OBJ_NEW(ompi_instance_collective_t);
    ompi_instance_collective_assign(coll, coll_procs, OMPI_FUNC_PMIX_QUERY, coll_params, NULL, coll_cbfunc, cbdata);

    coll->coll_cbfunc->type = OMPI_CBFUNC_INFO;
    
    ret = enter_collective_receiver(&ompi_instance_pending_collectives, coll, true);
...
/* Executes a non-blocking fence operation over the union of the specified PSets */
...
#pragma region v23
/* Create a basic rc op handle. Currently it describes an operation of type MPI_RC_ADD or MPI_RC_SUB */
...
static void ompi_instance_collective_infocb_send(   pmix_status_t status, 
		                                            pmix_info_t *results,
		                                            size_t nresults,
                                                    void *cbdata, 
                                                    pmix_release_cbfunc_t release_fn,
...
/* Query the runtime for available resource changes given either the delta PSet or the associated PSet */
int ompi_instance_get_res_change_collective(ompi_instance_t *instance, pmix_proc_t *coll_procs, size_t n_coll_procs, char *input_name, ompi_rc_op_type_t *type, char *output_name, int *incl, ompi_rc_status_t *status, opal_info_t **info_used, bool get_by_delta_name){
    int ret = OPAL_SUCCESS;
    char pset_search_name[OPAL_MAX_PSET_NAME_LEN];
    pmix_status_t rc;
    pmix_query_t query;
    ...
    if(NULL == (res_change = get_res_change_active_local(input_name))){
        ...
        if(is_leader){
            ...
            create_collective_query(&coll, coll_procs, n_coll_procs, &query, 1, NULL, 0);
            coll_cbfunc_create(&coll->coll_cbfunc, OMPI_CBFUNC_INFO);
            coll_cbfunc_load_info(coll->coll_cbfunc, ompi_instance_get_res_change_complete);
 
            coll->coll_cbdata = &lock;
            /*
             * TODO: need to handle this better
             */
            if (PMIX_SUCCESS != (rc = PMIx_Query_info_nb(&query, 1, 
